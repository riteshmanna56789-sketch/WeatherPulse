...
function requiredValue<T>(
  ...
  if (value === undefined || value === null) {
    throw new Error(
      `Open-Meteo response is missing ${label}.`
    )
  }
...
export function adaptWeatherResponse(
  ...
  const current = requiredValue(
    response.current,
    'current weather'
  )

  const hourly = requiredValue(
    response.hourly,
    'hourly forecast'
  )

  const daily = requiredValue(
    response.daily,
    'daily forecast'
  )

  const currentCode = requiredValue(
  ...
  const hourlyTimes = requiredValue(
    hourly.time,
    'hourly times'
  )

  const hourlyTemperatures = requiredValue(
    hourly.temperature_2m,
    'hourly temperatures'
  )

  const hourlyCodes = requiredValue(
    hourly.weather_code,
  ...
  const currentTime = requiredValue(
    current.time,
    'current time'
  )
      ...
      .map((time, index) => {
        ...
        return {
          ...
          temperature: Math.round(
            hourlyTemperatures[sourceIndex] ??
              hourlyTemperatures[0] ??
              0
          ),
          condition:
            describeWeatherCode(
              hourlyCodes[sourceIndex] ??
                currentCode
            ).condition,
          precipitationChance:
            hourlyPrecipitation[sourceIndex] ?? 0
        }
      })

  const dailyTimes = requiredValue(
    daily.time,
    'daily times'
  )

  const dailyCodes = requiredValue(
    daily.weather_code,
    'daily weather codes'
  )

  const dailyHighs = requiredValue(
    daily.temperature_2m_max,
    'daily high temperatures'
  )

  const dailyLows = requiredValue(
  ...
  const daylightDuration =
    dailyDaylightDuration[0]

  const daylight =
    sunrise &&
    sunset &&
    typeof daylightDuration === 'number'
      ? {
